...
Sprite3D* ThiccDrawNode3D::_drawLine(cocos2d::Vec3 start, cocos2d::Vec3 end, cocos2d::Color3B color, double thiccness)
{
    ...
    auto pitch = asin(u3n.y) * 180 / M_PI;
    double yaw;
    if (u2len == 0) {
        yaw = 0;
    }
...
void ThiccDrawNode3D::drawCircle(cocos2d::Vec3 center, double radius, cocos2d::Vec3 normal, int segments, cocos2d::Color4B color)
{
	Color3B cColor = Color3B(color.r, color.g, color.b);
	this->drawCircle(center, radius, normal, segments, cColor);
}
...
cocos2d::Sprite3D* ThiccDrawNode3D::getChildFromPool()
{
    ...
    if (this->_pool.size() > 0) {
        ...
        this->_pool.erase(this->_pool.begin());
