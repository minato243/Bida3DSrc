...
#define CushionCollisionFunc void(double, BallBody *, CushionSegment *)
...
namespace ps {
	...
	public:
		PhysicsDispatcher(PhysicsWorldV2 * world);
		~PhysicsDispatcher();

		void dispatchBallCollision(double timestamp, BallBody * ball_1, BallBody * ball_2);
		void dispatchCushionCollision(double timestamp, BallBody * ball, CushionSegment * cushion);
