...
Pockets * ps::PhysicsWorld::getPockets()
{
	return &_pockets;
}
...
void ps::PhysicsWorld::restoreStateFromData(const ShootResultData & data)
{
	for (int i = 0; i < _balls.size(); i++) {
		_balls[i]->reset();
		_balls[i]->disable();
	}
	for (int i = 0; i < data.ballsEnable.size(); i++) {
		auto d = data.ballsEnable[i];
		auto ball = getBall(d.ballId);
		ball->enable();
		ball->setPosition(d.pos);
	}
}

void ps::PhysicsWorld::onBallOutOfTable(BallBody * ball)
{
	//todo
...
void PhysicsWorld::onWorldPaused() {
	...
	Utility::writeToFile(("Debug_" + std::to_string(gameMgr->_userInfo->_uId)).c_str(), getWorldState().c_str());
}
...
PhysicSimulateResult * PhysicsWorld::shoot(int ballId, int cueSlot, double force, const vector & direction, const vector & offset) {
	char text[200];
	sprintf(text, "PhysicsWorld::shoot force %f, direction(%f %f %f), offset(%f %f)\n",
		force, direction.x, direction.y, direction.z, offset.x, offset.y);
...
std::string ps::PhysicsWorld::getWorldState()
{
	std::string state = "";
	for (auto i = 0; i < _balls.size(); i++) {
		char text[50];
		auto ball = _balls[i];
		sprintf(text, "ball %d (%f, %f, %f), ", ball->id(), ball->position().x, ball->position().y, ball->position().z);
		state += text;
	}
	state += "\n";
	return state;
