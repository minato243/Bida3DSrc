...
void main(void)
{
	...
	float DISTANCE_DIVIDE = ios ? 260.0 : 210.0;
	...
	for(int i = 0; i < u_light_count; i = i + 1)
	{
		...
		float distance = cur_distance/DISTANCE_DIVIDE;
	...
	color *= u_color.rgb;
	color.r = clamp(color.r, 0.0, 0.9999);
	color.g = clamp(color.g, 0.0, 0.9999);
	color.b = clamp(color.b, 0.0, 0.9999);
